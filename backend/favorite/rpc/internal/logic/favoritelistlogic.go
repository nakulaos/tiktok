...
func (l *FavoriteListLogic) FavoriteList(in *favorite.FavoriteListRequest) (*favorite.FavoriteListResponse, error) {
	...
	if _, err := l.svcCtx.UserModel.FindOne(l.ctx, userId); err != nil {
		if err == userModel.ErrNotFound {
			return nil, errors.Wrapf(errorcode.FavoriteUserIdEmptyError, "the user is not exist , id : %d", userId)
		} else {
	...
	if _, err := l.svcCtx.UserModel.FindOne(l.ctx, actorId); err != nil {
		if err == userModel.ErrNotFound {
			return nil, errors.Wrapf(errorcode.FavoriteUserIdEmptyError, "the user is not exist , id : %d", actorId)
		} else {
			return nil, errors.Wrapf(errorcode2.ServerError, "find user error, id : %d,err :%s", in.ActorId, err.Error())
		}
	}

	favorVideos, err := l.svcCtx.FavoriteModel.FindOwnFavorites(l.ctx, actorId)
	if err != nil {
	...
	for i := 0; i < len(favorVideos); i++ {
		...
		if err != nil {
			if err == videosModel.ErrNotFound {
				return nil, errors.Wrapf(errorcode.FavoriteVideoIdEmptyError, "video is not exist, id : %d", videoid)
			} else {
