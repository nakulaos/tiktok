// Code generated by protoc-gen-go-grpc. DO NOT EDIT.
// versions:
// - protoc-gen-go-grpc v1.3.0
// - protoc             v4.25.2
// source: user.proto

package user

import (
	context "context"
	grpc "google.golang.org/grpc"
	codes "google.golang.org/grpc/codes"
	status "google.golang.org/grpc/status"
)

// This is a compile-time assertion to ensure that this generated file
// is compatible with the grpc package it is being compiled against.
// Requires gRPC-Go v1.32.0 or later.
const _ = grpc.SupportPackageIsVersion7

const (
	UseSrv_LoginWithPhone_FullMethodName    = "/user.UseSrv/LoginWithPhone"
	UseSrv_LoginWithEmail_FullMethodName    = "/user.UseSrv/LoginWithEmail"
	UseSrv_LoginWithUsername_FullMethodName = "/user.UseSrv/LoginWithUsername"
	UseSrv_Register_FullMethodName          = "/user.UseSrv/Register"
	UseSrv_UserInfo_FullMethodName          = "/user.UseSrv/UserInfo"
	UseSrv_Update_FullMethodName            = "/user.UseSrv/Update"
)

// UseSrvClient is the client API for UseSrv service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type UseSrvClient interface {
	// 用户登录
	LoginWithPhone(ctx context.Context, in *LoginWithPhoneRequest, opts ...grpc.CallOption) (*LoginResponse, error)
	LoginWithEmail(ctx context.Context, in *LoginWithEmailRequest, opts ...grpc.CallOption) (*LoginResponse, error)
	LoginWithUsername(ctx context.Context, in *LoginWithUsernameRequest, opts ...grpc.CallOption) (*LoginResponse, error)
	// 用户注册
	Register(ctx context.Context, in *RegisterRequest, opts ...grpc.CallOption) (*RegisterResponse, error)
	// 用户信息
	UserInfo(ctx context.Context, in *UserInfoRequest, opts ...grpc.CallOption) (*UserInfoResponse, error)
	// 修改用户信息
	Update(ctx context.Context, in *UpdateReq, opts ...grpc.CallOption) (*UpdateResp, error)
}

type useSrvClient struct {
	cc grpc.ClientConnInterface
}

func NewUseSrvClient(cc grpc.ClientConnInterface) UseSrvClient {
	return &useSrvClient{cc}
}

func (c *useSrvClient) LoginWithPhone(ctx context.Context, in *LoginWithPhoneRequest, opts ...grpc.CallOption) (*LoginResponse, error) {
	out := new(LoginResponse)
	err := c.cc.Invoke(ctx, UseSrv_LoginWithPhone_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *useSrvClient) LoginWithEmail(ctx context.Context, in *LoginWithEmailRequest, opts ...grpc.CallOption) (*LoginResponse, error) {
	out := new(LoginResponse)
	err := c.cc.Invoke(ctx, UseSrv_LoginWithEmail_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *useSrvClient) LoginWithUsername(ctx context.Context, in *LoginWithUsernameRequest, opts ...grpc.CallOption) (*LoginResponse, error) {
	out := new(LoginResponse)
	err := c.cc.Invoke(ctx, UseSrv_LoginWithUsername_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *useSrvClient) Register(ctx context.Context, in *RegisterRequest, opts ...grpc.CallOption) (*RegisterResponse, error) {
	out := new(RegisterResponse)
	err := c.cc.Invoke(ctx, UseSrv_Register_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *useSrvClient) UserInfo(ctx context.Context, in *UserInfoRequest, opts ...grpc.CallOption) (*UserInfoResponse, error) {
	out := new(UserInfoResponse)
	err := c.cc.Invoke(ctx, UseSrv_UserInfo_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

func (c *useSrvClient) Update(ctx context.Context, in *UpdateReq, opts ...grpc.CallOption) (*UpdateResp, error) {
	out := new(UpdateResp)
	err := c.cc.Invoke(ctx, UseSrv_Update_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// UseSrvServer is the server API for UseSrv service.
// All implementations must embed UnimplementedUseSrvServer
// for forward compatibility
type UseSrvServer interface {
	// 用户登录
	LoginWithPhone(context.Context, *LoginWithPhoneRequest) (*LoginResponse, error)
	LoginWithEmail(context.Context, *LoginWithEmailRequest) (*LoginResponse, error)
	LoginWithUsername(context.Context, *LoginWithUsernameRequest) (*LoginResponse, error)
	// 用户注册
	Register(context.Context, *RegisterRequest) (*RegisterResponse, error)
	// 用户信息
	UserInfo(context.Context, *UserInfoRequest) (*UserInfoResponse, error)
	// 修改用户信息
	Update(context.Context, *UpdateReq) (*UpdateResp, error)
	mustEmbedUnimplementedUseSrvServer()
}

// UnimplementedUseSrvServer must be embedded to have forward compatible implementations.
type UnimplementedUseSrvServer struct {
}

func (UnimplementedUseSrvServer) LoginWithPhone(context.Context, *LoginWithPhoneRequest) (*LoginResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method LoginWithPhone not implemented")
}
func (UnimplementedUseSrvServer) LoginWithEmail(context.Context, *LoginWithEmailRequest) (*LoginResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method LoginWithEmail not implemented")
}
func (UnimplementedUseSrvServer) LoginWithUsername(context.Context, *LoginWithUsernameRequest) (*LoginResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method LoginWithUsername not implemented")
}
func (UnimplementedUseSrvServer) Register(context.Context, *RegisterRequest) (*RegisterResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Register not implemented")
}
func (UnimplementedUseSrvServer) UserInfo(context.Context, *UserInfoRequest) (*UserInfoResponse, error) {
	return nil, status.Errorf(codes.Unimplemented, "method UserInfo not implemented")
}
func (UnimplementedUseSrvServer) Update(context.Context, *UpdateReq) (*UpdateResp, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Update not implemented")
}
func (UnimplementedUseSrvServer) mustEmbedUnimplementedUseSrvServer() {}

// UnsafeUseSrvServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to UseSrvServer will
// result in compilation errors.
type UnsafeUseSrvServer interface {
	mustEmbedUnimplementedUseSrvServer()
}

func RegisterUseSrvServer(s grpc.ServiceRegistrar, srv UseSrvServer) {
	s.RegisterService(&UseSrv_ServiceDesc, srv)
}

func _UseSrv_LoginWithPhone_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LoginWithPhoneRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).LoginWithPhone(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_LoginWithPhone_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).LoginWithPhone(ctx, req.(*LoginWithPhoneRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _UseSrv_LoginWithEmail_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LoginWithEmailRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).LoginWithEmail(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_LoginWithEmail_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).LoginWithEmail(ctx, req.(*LoginWithEmailRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _UseSrv_LoginWithUsername_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(LoginWithUsernameRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).LoginWithUsername(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_LoginWithUsername_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).LoginWithUsername(ctx, req.(*LoginWithUsernameRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _UseSrv_Register_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(RegisterRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).Register(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_Register_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).Register(ctx, req.(*RegisterRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _UseSrv_UserInfo_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(UserInfoRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).UserInfo(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_UserInfo_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).UserInfo(ctx, req.(*UserInfoRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _UseSrv_Update_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(UpdateReq)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(UseSrvServer).Update(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: UseSrv_Update_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(UseSrvServer).Update(ctx, req.(*UpdateReq))
	}
	return interceptor(ctx, in, info, handler)
}

// UseSrv_ServiceDesc is the grpc.ServiceDesc for UseSrv service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var UseSrv_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "user.UseSrv",
	HandlerType: (*UseSrvServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "LoginWithPhone",
			Handler:    _UseSrv_LoginWithPhone_Handler,
		},
		{
			MethodName: "LoginWithEmail",
			Handler:    _UseSrv_LoginWithEmail_Handler,
		},
		{
			MethodName: "LoginWithUsername",
			Handler:    _UseSrv_LoginWithUsername_Handler,
		},
		{
			MethodName: "Register",
			Handler:    _UseSrv_Register_Handler,
		},
		{
			MethodName: "UserInfo",
			Handler:    _UseSrv_UserInfo_Handler,
		},
		{
			MethodName: "Update",
			Handler:    _UseSrv_Update_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "user.proto",
}
